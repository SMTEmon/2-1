...
## 2. Dimensions and Bases of the Subspaces
...
## 3. Finding a Basis for the Left Null Space
...
## 4. Visualizing the Subspaces (The "Strang Diagram")
...
graph LR
    %% Styles
    classDef rowspace fill:#a6cee3,stroke:#1f78b4,stroke-width:2px,color:#000;
    classDef nullspace fill:#b2df8a,stroke:#33a02c,stroke-width:2px,color:#000;
    classDef colspace fill:#fb9a99,stroke:#e31a1c,stroke-width:2px,color:#000;
    classDef leftnull fill:#fdbf6f,stroke:#ff7f00,stroke-width:2px,color:#000;
    classDef zero fill:#cccccc,stroke:#666666,stroke-width:2px,color:#000;

    %% Subgraph R^n
    subgraph Rn [Space R^n]
        direction LR
        RowSpace["Row Space C(A^T)<br>dim = r"]:::rowspace
        NullSpace["Null Space N(A)<br>dim = n - r"]:::nullspace
        RowSpace --- NullSpace
    end
    ...
    subgraph Rm [Space R^m]
        direction LR
        ColSpace["Column Space C(A)<br>dim = r"]:::colspace
        LeftNull["Left Null N(A^T)<br>dim = m - r"]:::leftnull
        ColSpace --- LeftNull
    end

    %% Mapping arrows
    Rn -->|Matrix A maps row space to column space| Rm
    NullSpace -->|A maps to| Zero["Zero Vector"]:::zero

```

---

## 5. New Concept: Matrices as a Vector Space
