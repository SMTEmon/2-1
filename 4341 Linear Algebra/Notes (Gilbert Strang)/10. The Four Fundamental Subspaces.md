...
## 1. Correction from Lecture 9
Professor Strang begins by correcting an error made in the previous lecture regarding basis vectors in $\mathbb{R}^3$.

*   **The Setup:**
    *   Vector $v_1 = \begin{bmatrix} 1 \\ 1 \\ 2 \end{bmatrix}$
    *   Vector $v_2 = \begin{bmatrix} 2 \\ 2 \\ 5 \end{bmatrix}$
    *   Vector $v_3 = \begin{bmatrix} 3 \\ 3 \\ 8 \end{bmatrix}$
*   **The Error:** He previously stated these formed a basis.
*   **The Correction:** These vectors are **linearly dependent**.
    *   Observation: Looking at the matrix formed by these columns, row 1 and row 2 are identical ($1, 2, 3$).
    *   Implication: If rows are dependent, columns must also be dependent. The determinant would be 0, and the matrix is not invertible.
    *   Conclusion: A matrix with identical rows (or dependent rows) cannot have independent columns. The rank is only 2, not 3.

---

## 2. The Four Fundamental Subspaces
This is the core concept of the course. For any $m \times n$ matrix $A$, there are four fundamental subspaces that dictate its behavior.
