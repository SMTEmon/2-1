...
A major property used in future lectures: The inverse of a transpose is the transpose of the inverse. The order of operations does not matter.
...
> [!example] Why Inverses don't mix
> If $E_{21}$ has a $-2$ (to kill pos 2,1) and $E_{32}$ has a $-5$ (to kill pos 3,2):
> *   The product $E$ might introduce a $10$ in the corner due to interaction ($(-2) \times (-5)$).
> *   The product $L$ (the inverses) keeps the $2$ and $5$ separate in their slots and does not create the "10". The multipliers sit unchanged in $L$.
